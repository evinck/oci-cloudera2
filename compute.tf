// Modified by eric.vinck@oracle.com on 12/06/24
...
module "bastion" {
	...
	bastion_instance_ocpus = "${var.bastion_instance_ocpus}"
	bastion_instance_mem = "${var.bastion_instance_mem}"
...
module "utility" {
	...
	utility_instance_ocpus = "${var.utility_instance_ocpus}"
	utility_instance_mem = "${var.utility_instance_mem}"
    log_volume_size_in_gbs = "${var.log_volume_size_in_gbs}"
    cloudera_volume_size_in_gbs = "${var.cloudera_volume_size_in_gbs}"
    user_data = "${base64encode(file("scripts/boot.sh"))}"
	cloudera_manager = "${data.null_data_source.values.outputs["cm_default"]}"
...
module "master" {
	...
	master_instance_ocpus = "${var.master_instance_ocpus}"
	master_instance_mem = "${var.master_instance_mem}"
    log_volume_size_in_gbs = "${var.log_volume_size_in_gbs}"
    cloudera_volume_size_in_gbs = "${var.cloudera_volume_size_in_gbs}"
    user_data = "${base64encode(file("scripts/boot.sh"))}"
	cloudera_manager = "${data.null_data_source.values.outputs["cm_default"]}"
...
module "worker" {
	...
	worker_instance_ocpus ="${var.worker_instance_ocpus}"
	worker_instance_mem = "${var.worker_instance_mem}"
	log_volume_size_in_gbs = "${var.log_volume_size_in_gbs}"
	cloudera_volume_size_in_gbs = "${var.cloudera_volume_size_in_gbs}"
